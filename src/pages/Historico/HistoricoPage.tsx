...
// ---------------------------------------------------------------------------
// Ciclos (mesmos da VacinaMembroPage)
// ---------------------------------------------------------------------------
...
function getCicloDeEntrada(entrada: EntradaTimeline): CicloId | 'avulsa' {
  ...
  if (entrada.avulsa) return 'avulsa'
...
// ---------------------------------------------------------------------------
// Dropdown de Ciclo (igual ao da VacinaMembroPage)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Helper de estilo dos botões de status (igual à VacinaMembroPage)
// ---------------------------------------------------------------------------
...
export function HistoricoPage() {
  ...
  // --- filtros aba avulsas ---
  const [filtroStatusAvulsa, setFiltroStatusAvulsa] = useState<'todos' | 'aplicada' | 'pendente' | 'atrasada'>('todos')
  const [buscaAvulsa, setBuscaAvulsa] = useState('')

  const membro = membros.find(m => m.id === membroId)

  // ---------------------------------------------------------------------------
  // Timeline completa (todas as entradas do membro)
  // ---------------------------------------------------------------------------
  const timeline = useMemo((): EntradaTimeline[] => {
    ...
    for (const reg of registrosMembro) {
      if (reg.vacina_id === 'avulsa') {
        entradas.push({
          tipo: 'aplicada',
          data: reg.data_aplicacao,
          vacinaNome: reg.observacoes ?? 'Vacina avulsa',
    ...
    for (const lem of lembretesMembro) {
      ...
      if (lem.tipo === 'manual') {
        ...
        entradas.push({
          ...
          data: dataLem,
          vacinaNome: lem.titulo ?? 'Vacina avulsa',
          numeroDose: lem.numero_dose ?? 1,
          faixas: [],
          avulsa: true,
        })
        continue
  ...
  // ---------------------------------------------------------------------------
  // Aba: Vacinas de Ciclo (exclui avulsas)
  // ---------------------------------------------------------------------------
  const timelineCiclo = useMemo(() => {
    return timeline.filter(e => !e.avulsa)
  }, [timeline])

  const timelineCicloFiltrada = useMemo(() => {
    return timelineCiclo.filter(e => {
  ...
  }, [timelineCiclo, filtroStatus, filtroCiclo, busca])

  // Separar por tipo (aba ciclo)
  const aplicadasCiclo = timelineCicloFiltrada.filter(e => e.tipo === 'aplicada')
  const futulasCiclo   = timelineCicloFiltrada.filter(e => e.tipo === 'pendente')
  const atrasadasCiclo = timelineCicloFiltrada.filter(e => e.tipo === 'atrasada')

  // Agrupar aplicadas por ciclo
  const aplicadasPorCiclo = useMemo(() => {
    const mapa = new Map<string, typeof aplicadasCiclo>()
    for (const e of aplicadasCiclo) {
    ...
    const grupos: {
      ...
      corBg: string
      corBorda: string
      entradas: typeof aplicadasCiclo
    }[] = []
  ...
  const timelineAvulsasFiltrada = useMemo(() => {
    return timelineAvulsas.filter(e => {
      if (filtroStatusAvulsa !== 'todos' && e.tipo !== filtroStatusAvulsa) return false
      if (buscaAvulsa) {
        const termo = buscaAvulsa.toLowerCase()
        const nomeMatch = e.vacinaNome.toLowerCase().includes(termo)
        const localMatch = e.tipo === 'aplicada' && ((e as { local?: string }).local ?? '').toLowerCase().includes(termo)
        if (!nomeMatch && !localMatch) return false
      }
  ...
  }, [timelineAvulsas, filtroStatusAvulsa, buscaAvulsa])
    ...
    <div style={{ maxWidth: 640, margin: '0 auto', paddingBottom: 'var(--space-8)' }}>
      ...
      {abaAtiva === 'ciclo' && (
        <>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-2)', marginBottom: 'var(--space-5)', alignItems: 'center' }}>
            ...
            <div style={{ flex: '1 1 180px', position: 'relative', minWidth: 160 }}>
              ...
              <input
                ...
                aria-label="Buscar vacina no histórico de ciclo"
              />
            </div>
          ...
          {/* Conteúdo aba ciclo */}
          {timelineCicloFiltrada.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 'var(--space-12) var(--space-8)', color: 'var(--color-text-muted)' }}>
              ...
              <p style={{ fontWeight: 600, marginBottom: 'var(--space-2)', color: 'var(--color-text)' }}>
                {timelineCiclo.length === 0 ? 'Nenhum registro ainda' : 'Nenhum resultado para os filtros'}
              </p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
                    ...
                    {atrasadasCiclo.map((e, i) => (
                      <li key={i} style={{ display: 'flex', gap: 'var(--space-4)', alignItems: 'flex-start' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 4 }}>
                          ...
                          {i < atrasadasCiclo.length - 1 && <div style={{ flex: 1, width: 2, background: 'var(--color-divider)', marginTop: 4, minHeight: 20 }} aria-hidden />}
                        </div>
              ...
              {futulasCiclo.length > 0 && (
                <section aria-label="Doses agendadas">
                  ...
                  <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }} role="list">
                    {futulasCiclo.map((e, i) => (
                      <li key={i} style={{ display: 'flex', gap: 'var(--space-4)', alignItems: 'flex-start' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 4 }}>
                          <Clock size={18} style={{ color: 'var(--color-primary)', flexShrink: 0 }} aria-hidden />
                          {i < futulasCiclo.length - 1 && <div style={{ flex: 1, width: 2, background: 'var(--color-divider)', marginTop: 4, minHeight: 20 }} aria-hidden />}
                        </div>
        ...
        <>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-2)', marginBottom: 'var(--space-5)', alignItems: 'center' }}>
            ...
            <div style={{ flex: '1 1 180px', position: 'relative', minWidth: 160 }}>
              ...
              <input
                type="search"
                placeholder="Buscar vacina avulsa..."
                value={buscaAvulsa}
                onChange={e => setBuscaAvulsa(e.target.value)}
                className="input-field"
                style={{ paddingLeft: 'var(--space-8)', minHeight: 40, fontSize: 'var(--text-sm)' }}
                aria-label="Buscar vacina avulsa no histórico"
              />
            </div>

            {/* Botões de status avulsas */}
            ...
            <button onClick={() => setFiltroStatusAvulsa('pendente')} style={styleBtnStatus('pendente', filtroStatusAvulsa === 'pendente', 'var(--color-primary)')}>
              Agendadas
            </button>
            <button onClick={() => setFiltroStatusAvulsa('atrasada')} style={styleBtnStatus('atrasada', filtroStatusAvulsa === 'atrasada', 'var(--color-error)')}>
              Atrasadas
            </button>

            {/* Limpar filtros avulsas */}
            {(filtroStatusAvulsa !== 'todos' || buscaAvulsa) && (
              <button
                onClick={() => { setFiltroStatusAvulsa('todos'); setBuscaAvulsa('') }}
                style={{ ...styleBtnStatus('limpar', false), color: 'var(--color-error)', borderColor: 'var(--color-error-highlight)', display: 'flex', alignItems: 'center', gap: 'var(--space-1)' }}
                aria-label="Limpar filtros de avulsas"
              >
                <X size={12} aria-hidden /> Limpar
              </button>
            )}
          ...
          {/* Conteúdo aba avulsas */}
          {timelineAvulsasFiltrada.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 'var(--space-12) var(--space-8)', color: 'var(--color-text-muted)' }}>
              <ClipboardList size={40} style={{ margin: '0 auto var(--space-3)', opacity: 0.3 }} aria-hidden />
              <p style={{ fontWeight: 600, marginBottom: 'var(--space-2)', color: 'var(--color-text)' }}>
                {timelineAvulsas.length === 0 ? 'Nenhuma vacina avulsa registrada' : 'Nenhum resultado para os filtros'}
              </p>
              <p style={{ fontSize: 'var(--text-xs)', maxWidth: 280, margin: '0 auto' }}>
                {timelineAvulsas.length === 0
                  ? 'Vacinas fora do calendário oficial (viagens, indicação médica, etc.) aparecem aqui.'
                  : 'Tente ajustar os filtros ou a busca.'}
              </p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
              {timelineAvulsasFiltrada.map((e, i) => {
                const isAtrasada = e.tipo === 'atrasada'
                const isPendente = e.tipo === 'pendente'
                const isAplicada = e.tipo === 'aplicada'
                const cor = isAtrasada ? 'var(--color-error)' : isPendente ? 'var(--color-primary)' : 'var(--color-success)'
                const corBg = isAtrasada ? 'var(--color-error-highlight)' : isPendente ? 'var(--color-primary-highlight)' : 'var(--color-success-highlight)'
                const labelTipo = isAtrasada ? 'Atrasada' : isPendente ? 'Agendada' : 'Aplicada'
                const Icone = isAtrasada ? AlertCircle : isPendente ? Clock : CheckCircle2
                ...
                return (
                  ...
                  >
                    <Icone size={18} style={{ color: cor, flexShrink: 0 }} aria-hidden />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
                        <p style={{ fontWeight: 700, fontSize: 'var(--text-sm)', color: 'var(--color-text)' }}>{e.vacinaNome}</p>
                        <span style={{
                      ...
                      <p style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', marginTop: 'var(--space-1)' }}>
                        Dose {e.numeroDose} · {e.data ? formatarData(e.data) : '—'}
                        {isAplicada && (e as { local?: string }).local ? ` · ${(e as { local?: string }).local}` : ''}
                        {isAtrasada ? ' · Não realizada' : ''}
                      </p>
                    </div>
                  </div>
                )
